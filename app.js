...
var normalizeData = function (unnormalizedData) {
    // Your code here...
    return unnormalizedData.reduce(function (acc, post) {
        // Add the post to the byId object, using the post's id as the key
        acc.byId[post.id] = post;
        // Add the post's id to the allIds array
        acc.allIds.push(post.id);
        return acc;
    }, { byId: {}, allIds: [] }); //Initialize the acc with an empty byId object and allIds array
};
